...
class BlockHeaderAPI(MiningHeaderAPI, BlockHeaderSedesAPI):
    """
    A class derived from :class:`~veda.abc.MiningHeaderAPI` to define a block header
    after it is sealed.
    """
...
class VedaBlockHeaderAPI(BlockHeaderAPI):

    @property
    @abstractmethod
    def veda_block_hash(self) -> Hash32:
        """
        Return the hash of the BTC block header.
        """
        ...
    ...
    def veda_timestamp(self) -> int:
        """
        Return the timestamp of the BTC block header.
        """
        ...

    @property
    @abstractmethod
    def veda_block_number(self) -> int:
        """
        Return the block number of the BTC block header.
        """
        ...
...
class TransactionFieldsAPI(ABC):
    ...
    @property
    @abstractmethod
    def veda_sender(self) -> Address:
        ...


    @property
    @abstractmethod
    def veda_txhash(self) -> Hash32:
        ...
...
class HeaderDatabaseAPI(ABC):
    ...
    def __init__(self, db: AtomicDatabaseAPI) -> None:
        """
        Instantiate the database from an :class:`~veda.abc.AtomicDatabaseAPI`.
        """
        ...
...
class ChainDatabaseAPI(HeaderDatabaseAPI):
    ...
    :class:`~veda.abc.HeaderDatabaseAPI`.
    """
    ...
    def persist_block(
        ...
        .. warning::
            ...
            :meth:`veda.abc.ChainDatabaseAPI.persist_unexecuted_block` to persist blocks
    ...
    def persist_unexecuted_block(
        ...
        **not** validate if state transitions are valid.
        ...
        transactions which :meth:`veda.abc.ChainDatabaseAPI.persist_block` in contrast
...
class MemoryAPI(ABC):
    """
    A class representing the memory of the :class:`~veda.abc.VirtualMachineAPI`.
    """
...
class StackAPI(ABC):
    """
    A class representing the stack of the :class:`~veda.abc.VirtualMachineAPI`.
    """
    ...
    def pop1_bytes(self) -> bytes:
        ...
        Raise `veda.exceptions.InsufficientStack` if the stack was empty.
        """
        ...
    ...
    def pop_any(self, num_items: int) -> Tuple[Union[int, bytes], ...]:
        ...
        Raise `veda.exceptions.InsufficientStack` if there are not enough items on
        the stack.
...
class ComputationAPI(
    ...
    def error(self) -> VMError:
        """
        Return the :class:`~veda.exceptions.VMError` of the computation.
    ...
    def error(self, value: VMError) -> None:
        """
        Set an :class:`~veda.exceptions.VMError` for the computation.
        """
    ...
    def extend_memory(self, start_position: int, size: int) -> None:
        ...
        bytes in length.  Raise `veda.exceptions.OutOfGas` if there is not enough
    ...
    def consume_gas(self, amount: int, reason: str) -> None:
        ...
        Raise `veda.exceptions.OutOfGas` if there is not enough gas remaining.
        """
        ...
...
class AccountDatabaseAPI(ABC):
    ...
    def lock_changes(self) -> None:
        ...
        See :meth:`veda.abc.AccountStorageDatabaseAPI.lock_changes` for
...
class TransactionExecutorAPI(ABC):
    ...
    def __call__(self, transaction: SignedTransactionAPI) -> "ComputationAPI":
        """
        Execute the ``transaction`` and return a :class:`veda.abc.ComputationAPI`.
        """
        ...
    ...
    def build_evm_message(self, transaction: SignedTransactionAPI) -> MessageAPI:
        """
        Build and return a :class:`~veda.abc.MessageAPI` from the given ``transaction``.
        """
        ...
...
class StateAPI(ConfigurableAPI):
    ...
    Each :class:`~veda.abc.VirtualMachineAPI` must be configured with a subclass of the
    :class:`~veda.abc.StateAPI`.

      .. note::

        Each :class:`~veda.abc.StateAPI` class must be configured with:

        - ``computation_class``: The :class:`~veda.abc.ComputationAPI` class for
          vm execution.
        - ``transaction_context_class``: The :class:`~veda.abc.TransactionContextAPI`
    ...
    def get_account_db_class(cls) -> Type[AccountDatabaseAPI]:
        """
        Return the :class:`~veda.abc.AccountDatabaseAPI` class that the
        state class uses.
        """
        ...
    ...
    def get_transaction_context_class(cls) -> Type[TransactionContextAPI]:
        """
        Return the :class:`~veda.vm.transaction_context.BaseTransactionContext` class
    ...
    def get_transaction_context(
        ...
        """
        Return the :class:`~veda.abc.TransactionContextAPI` for the given ``transaction``
        """
        ...

class VirtualMachineAPI(ConfigurableAPI):
    """
    The :class:`~veda.abc.VirtualMachineAPI` class represents the Chain rules for a
    specific protocol definition such as the Frontier or Homestead network.

      .. note::

        Each :class:`~veda.abc.VirtualMachineAPI` class must be configured with:

        - ``block_class``: The :class:`~veda.abc.BlockAPI` class for blocks in this
            VM ruleset.
        - ``_state_class``: The :class:`~veda.abc.StateAPI` class used by this
            VM for execution.
    ...
    def apply_transaction(
        ...
        :func:`~veda.vm.state.State.apply_transaction` with some extra
        orchestration logic.
    ...
    def create_execution_context(
        ...
        """
        Create and return the :class:`~veda.abc.ExecutionContextAPI`` for the given
    ...
    def generate_block_from_parent_header(
        ...
        """
        Generate block from parent header.
        """
        ...
    ...
    def get_block_class(cls) -> Type[BlockAPI]:
        """
        Return the :class:`~veda.rlp.blocks.Block` class that this VM uses for blocks.
        """
        ...
    ...
    def validate_header(
        ...
        """
        :raise veda.exceptions.ValidationError: if the header is not valid
        """
        ...
...
class HeaderChainAPI(ABC):
    """
    Like :class:`veda.abc.ChainAPI` but does only support headers, not entire blocks.
    """
...
class ChainAPI(ConfigurableAPI):
    ...
    def get_chaindb_class(cls) -> Type[ChainDatabaseAPI]:
        """
        Return the class for the used :class:`~veda.abc.ChainDatabaseAPI`.
        """
        ...
    ...
    def get_block_by_hash(self, block_hash: Hash32) -> BlockAPI:
        ...
        :raise veda.exceptions.HeaderNotFound: if the header is missing
        :raise veda.exceptions.BlockNotFound: if any part of the block body is missing
        """
        ...
    ...
    @abstractmethod
    def build_block_with_transactions(self, transactions: Tuple[SignedTransactionAPI, ...],
    ...
    def validate_chain_extension(self, headers: Tuple[BlockHeaderAPI, ...]) -> None:
        ...
        :meth:`~veda.abc.ChainAPI.validate_chain`, it is an additional API to call at a
...
class MiningChainAPI(ChainAPI):
    """
    Like :class:`~veda.abc.ChainAPI` but with APIs to create blocks incrementally.
    """
    ...
    def mine_block(self, *args: Any, **kwargs: Any) -> BlockAPI:
        ...
        See VM. :meth:`~veda.vm.base.VM.mine_block`
        """
        ...
