...
from veda._utils.os import friendly_filename_or_url
...
from veda.config import BaseAppConfig, VedaAppConfig, VedaConfig
from veda.constants import APP_IDENTIFIER_ETH1, SYNC_FULL
from veda.initialization import initialize_data_dir, is_data_dir_initialized

if TYPE_CHECKING:
    from veda.extensibility.trio import TrioIsolatedComponent  # noqa: F401
    from veda.extensibility.asyncio import AsyncioIsolatedComponent  # noqa: F401

TReturn = TypeVar("TReturn")
logger = logging.getLogger('veda.extensibility.component')
...
class BaseComponentAPI(ABC):
    ...
    def configure_parser(cls, arg_parser: ArgumentParser, subparser: _SubParsersAction) -> None:
        """
        Give the component a chance to amend the Veda CLI argument parser.
        """
        ...
...
class BaseComponent(ComponentAPI):
    ...
    # name.
    logger = get_logger('veda.components.BaseComponent')

    def __init__(self, boot_info: BootInfo) -> None:
        if not hasattr(self, 'name'):
            raise AttributeError(f"No name attribute defined for {self.__class__}")
        self.logger = get_logger(f'veda.components.{self.name}')
...
class BaseIsolatedComponent(BaseComponent):
    """
    A :class:`~veda.extensibility.component.BaseIsolatedComponent` runs in an isolated process
...
def _setup_standalone_component(
    ...
    if app_identifier == APP_IDENTIFIER_ETH1:
        app_cfg: Type[BaseAppConfig] = VedaAppConfig
    else:
    ...
    veda_config = VedaConfig.from_parser_args(args, app_identifier, (app_cfg,))
...
async def _run_eventbus_for_component(
    ...
    from veda.extensibility.trio import TrioIsolatedComponent
    from veda.extensibility.asyncio import AsyncioIsolatedComponent
    ...
    veda_config = component._boot_info.veda_config
    conn_config = ConnectionConfig.from_name(
        component.get_endpoint_name(), veda_config.ipc_dir)
...
def run_asyncio_veda_component(component_type: Type['AsyncioIsolatedComponent']) -> None:
    import asyncio
    from veda._utils.asyncio_utils import wait_first
...
def run_trio_veda_component(component_type: Type['TrioIsolatedComponent']) -> None:
