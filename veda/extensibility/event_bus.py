...
from veda.events import AvailableEndpointsUpdated, EventBusConnected


class BaseEventBusService(Service):
    logger = logging.getLogger('veda.extensibility.event_bus.EventBusService')
    ...
    async def run(self) -> None:
        async with self.EndpointType.serve(self._connection_config) as endpoint:
            ...
            main_endpoint_config = ConnectionConfig.from_name(
                MAIN_EVENTBUS_ENDPOINT, self._veda_config.ipc_dir
            )
