...
from veda.rpc.rlp.sedes import UninterpretedTransaction


class DefaultTransactionValidator():
    """
    The :class:`~veda.tx_pool.validators.DefaultTransactionValidator` class is responsible to
    ...
    def from_network_id(cls, chain: ChainAPI, network_id: int) -> 'DefaultTransactionValidator':
        if network_id == VEDA_NETWORK_ID:
            return cls(chain, BlockNumber(0))
        else:
