...
from veda.extensibility import Application
from veda._utils.ipc import (
...
class FixUncleanShutdownComponent(Application):
    logger = logging.getLogger('veda.components.fix_unclean_shutdown.FixUncleanShutdown')
    ...
    @classmethod
    def fix_unclean_shutdown(cls, args: Namespace, veda_config: VedaConfig) -> None:
        cls.logger.info("Cleaning up unclean shutdown...")

        cls.logger.info("Searching for process id files in %s...", veda_config.data_dir)
        pidfiles = tuple(veda_config.pid_dir.glob('*.pid'))
        if len(pidfiles) > 1:
        ...
        remove_dangling_ipc_files(cls.logger, veda_config.ipc_dir)
