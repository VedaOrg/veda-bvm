...
from veda.consensus.applier import (
    ConsensusApplier,
)
from veda.consensus.noproof import (
    NoProofConsensus,
)
from veda.db.atomic import (
    AtomicDB,
)
from veda.db.backends.memory import (
    MemoryDB,
)
from veda.rlp.headers import (
    HeaderParams,
)
from veda.tools._utils.mappings import (
    deep_merge,
)
from veda.tools._utils.normalization import (
    normalize_state,
)
from veda.tools.mining import (
    POWMiningMixin,
)
...
def build(obj: Any, *applicators: Callable[..., Any]) -> Any:
    ...
    If ``obj`` is an instances of :class:`~veda.chains.base.BaseChain` the
...
def fork_at(
    ...
    .. code-block:: python
        ...
        from veda.tools.builder.chain import build, fork_at
    ...
    * :func:`~veda.tools.builder.chain.frontier_at`
    * :func:`~veda.tools.builder.chain.homestead_at`
    * :func:`~veda.tools.builder.chain.tangerine_whistle_at`
    * :func:`~veda.tools.builder.chain.spurious_dragon_at`
    * :func:`~veda.tools.builder.chain.byzantium_at`
    * :func:`~veda.tools.builder.chain.constantinople_at`
    * :func:`~veda.tools.builder.chain.petersburg_at`
    * :func:`~veda.tools.builder.chain.istanbul_at`
    * :func:`~veda.tools.builder.chain.muir_glacier_at`
    * :func:`~veda.tools.builder.chain.berlin_at`
    * :func:`~veda.tools.builder.chain.london_at`
    * :func:`~veda.tools.builder.chain.arrow_glacier_at`
    * :func:`~veda.tools.builder.chain.gray_glacier_at`
    * :func:`~veda.tools.builder.chain.latest_mainnet_at` - whatever latest mainnet VM is
    """
...
def disable_dao_fork(chain_class: Type[ChainAPI]) -> Type[ChainAPI]:
    ...
    :class:`~veda.vm.forks.homestead.HomesteadVM`.  Requires that presence of
    the :class:`~veda.vm.forks.homestead.HomesteadVM`  in the
    ``vm_configuration``
    """
...
def dao_fork_at(
    ...
    version of the :class:`~veda.vm.forks.homestead.HomesteadVM` is present in
...
def mine_blocks(num_blocks: int, chain: MiningChainAPI) -> MiningChainAPI:
    """
    Variadic argument version of :func:`~veda.tools.builder.chain.mine_block`
    """
    if not isinstance(chain, MiningChainAPI):
        raise ValidationError("`mine_block` may only be used on MiningChain instances")
...
def import_blocks(*blocks: BlockAPI) -> Callable[[ChainAPI], ChainAPI]:
    """
    Variadic argument version of :func:`~veda.tools.builder.chain.import_block`
    """
