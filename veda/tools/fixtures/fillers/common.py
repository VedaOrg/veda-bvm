...
from veda.tools._utils.vyper import (
    compile_vyper_lll,
)
...
def expect(
    ...
    no transaction.
    (here, its role is played by :func:`~veda.tools.fixtures.fillers.execution`).

    * ``post_state`` is a list of state definition in the same form as expected
      by :func:`~veda.tools.fixtures.fillers.pre_state`. State items that are
